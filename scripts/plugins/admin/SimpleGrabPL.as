...
const array<string> allowed = {
'STEAM_0:0:0', // example
};

int canGrab(CBasePlayer@ pPlayer)
{
  if( pPlayer !is null )
  {
    if( g_PlayerFuncs.AdminLevel(pPlayer) < ADMIN_YES )
    {
      if( allowed.find(g_EngineFuncs.GetPlayerAuthId(pPlayer.edict())) >= 0)
      {
        return 2;
      }
    }
...
void Grab( const CCommand@ pArgs )
{
	...
	else
	{
		...
		{
			if (canGrab(pPlayer) >= 2 && pEntity.IsBSPModel())
				return;
