...
const float g_flMaxDistance = 16;	//How far a player can be from the spray origin and still get info

const int g_iMessageLifeTime = 0;	//How many seconds to keep showing the message after the player has stopped looking at a spray
			...
			if( tr.flFraction < 1.0 )
			{
				...
				if( pNearest !is null )
				{
					...
					if( ( pNearest.Position - tr.vecEndPos ).Length() <= g_flMaxDistance )
					{
						...
						snprintf( szMessage, "--%1", pNearest.PlayerName);
