...
HookReturnCode PlayerKilled( CBasePlayer@ pPlayer, CBaseEntity@ pAttacker, int iGib ) {
	...
	switch ( Math.RandomLong( 0, 10 ) ) {
		...
		case 10:
		{
			...
			g_Scheduler.SetTimeout( "KillHelper", 240, EHandle(helper) );
		}
		break;
	}
...
void KillHelper( EHandle& in helper ) {
	if ( helper.IsValid() ) {
		...
		if ( killme.IsAlive() && killme.pev.health > 0 ) {
			killme.TakeDamage( g_EntityFuncs.Instance(0).pev, g_EntityFuncs.Instance(0).pev, 75, DMG_BLAST );
			
			if ( killme.IsPlayer() )
				g_SoundSystem.EmitSound( cast<CBasePlayer@>( killme ).edict(), CHAN_AUTO, g_painsound, 0.75f, ATTN_STATIC );
		}
...
void CreateEffect( CBasePlayer@ pPlayer ) {
	...
	g_SoundSystem.EmitSound( pPlayer.edict(), CHAN_ITEM, g_effectsound, 0.75f, ATTN_STATIC );
}
