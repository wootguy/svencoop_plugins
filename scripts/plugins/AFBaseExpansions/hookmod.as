...
class HookMod : AFBaseClass
{
	...
	void MapInit()
	{
		...
		g_Game.PrecacheGeneric("sprites/afbhookbeam.spr");
		g_Game.PrecacheModel("sprites/afbhookbeam.spr");
				...
				if(pSearch !is null)
				{
					if(g_hookingPlayers.exists(pSearch.entindex()))
					{
						...
						NetworkMessage message(MSG_BROADCAST, NetworkMessages::SVC_TEMPENTITY, null);
							...
							message.WriteShort(g_EngineFuncs.ModelIndex("sprites/afbhookbeam.spr"));
							message.WriteByte(1);
							message.WriteByte(1);
							message.WriteByte(2);
							message.WriteByte(8);
						...
						float fTotal = 600.0f/sqrt(fPow); // 600
						for(int j = 0; j < 3; j++)
							velocity[j] *= fTotal;
